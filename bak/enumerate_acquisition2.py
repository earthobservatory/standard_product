...
def enumerate_acq(acq):
    ...
    for ref_scene in ref_scenes:
        ...
        if ref_dt_orb not in orbit_dict:
            ...
            logger.info("REF_DT_ORB : %s VALUE : %s"%(ref_dt_orb, orbit_dict[ref_dt_orb]))
                ...
                for matched_day, matched_ids in matched_days.items():
                    ...
                    if matched_dt_orb not in orbit_dict:
                        ...
                        orbit_dict[matched_dt_orb] = fetch("%s.0" % matched_dts[0].isoformat(),
                                                           ...
                                                           mission=mission, dry_run=True)
  			logger.info("REF_DT_ORB : %s VALUE : %s"%(matched_dt_orb, orbit_dict[matched_dt_orb]))
  				...
  				logger.info("MASTER_ZIP_URLS : %s" %master_zip_urls)
				logger.info("MASTER_ORBIT_URLS : %s" %master_orbit_urls)
				logger.info("SLAVE_ZIP_URLS : %s" %slave_zip_urls)
				logger.info("SLAVE_ORBIT_URLS : %s" %slave_orbit_urls)
				'''
