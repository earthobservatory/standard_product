    ...
    for track_dt in sorted(selected_track_acqs.keys(), reverse=True):
        ...
        for slave_track_dt in sorted( slave_grouped_matched["grouped"][track], reverse=True):
            ...
            if orbit_file:
                ...
                selected = gtUtil.water_mask_check(slave_track_dt, slave_grouped_matched["acq_info"], slave_grouped_matched["grouped"][track][slave_track_dt],  aoi_location, orbit_file)
                if not selected:
                ...
                selected = gtUtil.water_mask_check(slave_track_dt, slave_grouped_matched["acq_info"], slave_grouped_matched["grouped"][track][slave_track_dt],  aoi_location)
                if not selected:
                    logger.info("Removing the acquisitions of orbitnumber : %s for failing water mask test" %slave_track_dt)
                    rejected_slave_track_dt.append(slave_track_dt)
                    continue
...
def get_candidate_pair_list_by_orbitnumber(track, selected_track_acqs, aoi_data, orbit_data, reject_pairs):
    ...
    for orbitnumber in sorted(selected_track_acqs.keys(), reverse=True):
        ...
        for slave_orbitnumber in sorted( slave_grouped_matched["grouped"][track], reverse=True):
            selected_slave_acqs=[]
            selected = gtUtil.water_mask_check(slave_orbitnumber, slave_grouped_matched["acq_info"], slave_grouped_matched["grouped"][track][slave_orbitnumber],  aoi_location)
            if not selected:
