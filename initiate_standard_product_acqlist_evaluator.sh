...
echo "##########################################" 1>&2
echo -n "Running initiate_standard_product_acqlist_evaluator: " 1>&2
date 1>&2
/usr/bin/python3 $BASE_PATH/initiate_standard_product_acqlist_evaluator.py > initiate_standard_product_acqlist_evaluator.log 2>&1
STATUS=$?
echo -n "Finished running initiate_standard_product_acqlist_evaluator: " 1>&2
date 1>&2
if [ $STATUS -ne 0 ]; then
  echo "Failed to run initiate_standard_product_acqlist_evaluator." 1>&2
  cat initiate_standard_product_acqlist_evaluator.log 1>&2
  echo "{}"
  exit $STATUS
fi
